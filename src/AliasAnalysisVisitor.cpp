...
void AliasAnalysisVisitor::visitSelectInst(SelectInst &I) {
    ...
    auto lreg = currCtx->findOpReg(lhs);
    auto rreg = currCtx->findOpReg(rhs);
    if (hasPointsTo(lreg) || hasPointsTo(rreg)) {
        auto dstreg = currCtx->getDestReg(&I);
...
void AliasAnalysisVisitor::visitLoadInst(LoadInst &I) {
    ...
    for (auto &pt: srcreg->pointsto) {
        ...
        } else if (ensure2Lpt && !designated_mallocobj.size()) {
...
void AliasAnalysisVisitor::visitLibFunctions(CallInst &I, Function *func) {
    ...
    if (func->getName().equals("malloc") && designated_mallocobj.size()) {
        auto dstreg = currCtx->getDestReg(&I);
        for (auto reg: designated_mallocobj) {
            dstreg->mergePointsTo(reg, &I);
            for (auto &pt: dstreg->pointsto) {
                auto field = pt.target->findFieldObj(pt.dstoff);
                if (field) field->ignoresink = true;
            }
        }
        return;
    }
...
void AliasAnalysisVisitor::setupChildContext(CallInst &I, AliasTaintContext *parentContext) {
    ...
    auto func2enter = currCtx->func;
    if (func2enter->getName().equals("BN_POOL_get")
            || func2enter->getName().equals("bn_expand_internal")) {
        auto val = func2enter->arg_begin();
        auto reg = currCtx->findOpReg(val);
        for (auto &pt: reg->pointsto) {
            auto field = pt.target->findFieldObj(pt.dstoff);
            if (hasPointsTo(field)) {
                designated_mallocobj.insert(field);
            }
        }
    }
}
...
void AliasAnalysisVisitor::stitchChildContext(CallInst &I, AliasTaintContext *childContext) {
    auto func2exit = childContext->func;
    bool is_pool_malloc =
            func2exit->getName().equals("BN_POOL_get") ||
            func2exit->getName().equals("bn_expand_internal");
    RegObject *dstreg = nullptr;
    for (auto val: childContext->retval) {
        ...
        if (hasPointsTo(srcreg)) {
            ...
            for (auto &item: srcreg->pointsto) {
                if (!is_pool_malloc || !item.target->fake) {
                    auto tmp = item;
                    tmp.propagator = &I;
