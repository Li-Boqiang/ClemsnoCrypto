...
CallInst* insertMemset(Module &M, Value *target, Instruction *insertbefore) {
    ...
    CallInst *Set0CI = builder.CreateMemSet(
            I, IntZero, allocasize, MaybeAlign(I->getAlignment()));
    return Set0CI;
}
...
ExpandFuncPtr::ExpandFuncPtr(CallInst *inst) {
    originst = inst;
    funcptr = inst->getCalledValue();
    fptype = inst->getFunctionType();
    F = inst->getFunction();
    M = inst->getModule();
    for (auto &use: inst->arg_operands()) {
...
Function *funcwrap(Function *target, std::string wrapper_name, CallInst *callinst, bool callerprotect) {
    auto M = target->getParent();
    auto wrapper = M->getFunction(wrapper_name);
    if (!wrapper) {
        wrapper = cast<Function>(M->getOrInsertFunction(wrapper_name, target->getFunctionType()).getCallee());
        auto entryBB = BasicBlock::Create(wrapper->getContext(), "entryBB", wrapper);
        ...
        auto newcall = builder.CreateCall(functype, target, args);
        if (functype->getReturnType()->isVoidTy())
            builder.CreateRet(nullptr);
        else
            builder.CreateRet(newcall);
        if (callerprotect) {
            insertWRPKRU(*M, newcall, 0);
            insertWRPKRU(*M, newcall->getNextNode(), 3);
        }
    } 
    callinst->setCalledFunction(wrapper);
    return wrapper;
}
